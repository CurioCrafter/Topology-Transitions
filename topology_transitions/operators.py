...
class QT_OT_apply_transition(Operator):
    ...
    bl_description = (
        "Replace the selected rectangular quad patch with an all-quad transition"
    )
    ...
    def execute(self, context: Any):
        ...
        self.report(
            ...
            f"{stats['new_faces']} quads, {stats['poles']} N-poles, boundary preserved",
        )
        if stats["had_uvs"]:
            self.report(
                {"WARNING"},
...
class QT_OT_validate_patch(Operator):
    ...
    def execute(self, context: Any):
        ...
        self.report(
            {"INFO"},
            f"Valid {layout.width} x {layout.height} patch for "
